...
def plot(df, nm):
    meta = {'title':f'Spectroscope Data {nm}',
    ...
    data = tk.fit_data(df['wavelength'], df['reading'], fit_type='custom', 
                            model_function_custom=equation, chi=True, res=True,
                            yerr=df['uncert'], guess=(5e3, 3e-1))
    print(data['popt'], data['pstd'])
    tk.plot_data(df['wavelength'], df['reading'], 
                            data['plotx'], data['ploty'], 
                            data['residuals'], yerr=df['uncert'],
                            meta=meta)
